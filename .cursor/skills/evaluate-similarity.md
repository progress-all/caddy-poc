...
summaryには、**差分項目のみ**を**日本語の箇条書き**で出力してください。見出し・タイトル・前置き文は一切出力しない。

### 出力ルール（重要）
- 見出し・タイトル・前置き文は一切出力しない
- 各行は必ず「・」から始める（日本語の箇条書き）
- 差分項目のみを出力する（同等項目は一切出力しない）
- 各項目は1行で完結させる
- 「→」を用いて「元部品（Target）→ 比較対象部品（Candidate）」の順で表記する
- 数値・単位は元データの値をそのまま使用する
- 数値差分が明確でない場合は「条件・特性に差あり」と簡潔に記載する
- 英語は使用しない
- 文章形式（〜である、〜となっている 等）は使用しない

### 判定ルール（表示対象に含める）
- score 1–84 の項目（数値が異なる、規格・条件が異なる、明確に意味差がある特性）

### 判定ルール（表示対象から除外する）
- 完全一致の項目（score 100）
- 表記揺れのみの項目（score 100）
- 数学的・論理的に同義な項目（score 100）
- 上位互換・実用上問題なし（score 85–99）
- 比較不能（score 0。Refer to URL、別表参照、グラフ参照 等）
...
### 出力形式
- 複数行になる場合は改行で区切る（JSON文字列内では \n で連結）
- 余分な説明文・改行・まとめ文は出力しない

### 良い例（summary の内容）
```
・厚さ：0.8mm → 0.5mm
・絶縁抵抗規格：500Ω·F → 50Ω·F
・静電容量の測定電圧：1.0Vrms → 0.5Vrms
・誘電正接・充電時間：条件・特性に差あり
```
...
### 悪い例
- "比較結果の差分は以下のとおり。" などの前置き
- "厚さが異なる(0.8mm→0.5mm)。" などの文形式
- "要確認""確認が必要" などのアクション促す表現
- ・で始まらない行や、差分以外の説明を混ぜる
